 ...
 * @LastEditTime: 2023-02-17 16:19:59
...
export default defineComponent({
  ...
  setup(props: TreeProps) {
    ...
    const renderNode = (item: TreeItem) => {
      ...
      return (
        <div
          class={['lcui-tree-node', item.open && 'lcui-tree-node__open']}
          style={{ paddingLeft: `${24 * (itemLevel - 1)}px` }}
        >
          <div class='lcui-tree-node__content'>
            <div class='lcui-tree-node__content--value-wrapper'>
              {item.children ? (
                item.open ? (
                  <IconOpen
                    class='mr-xs'
              ...
              <span class='lcui-tree-node__title'>{item.label}</span>
            </div>
          </div>
        </div>
      );
